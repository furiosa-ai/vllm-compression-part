...
"""
MXFP8 (Microscaling FP8) scheme for compressed-tensors models.

Handles models with:
  - format: mxfp8-quantized
  - weight strategy: group (group_size=32)
  - scale_dtype: uint8 (E8M0 exponent format)

Weight dequantization and activation fake-quantization are performed
using the compressed-tensors library APIs (no CUDA extension dependency).
"""
...
class CompressedTensorsW8A8MXFp8(CompressedTensorsScheme):
    """
    Emulation-mode MXFP8 W8A8 scheme.

    Weights are stored as float8_e4m3fn with uint8 E8M0 per-group scales.
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        # E8M0 uint8 scales -> bfloat16 power-of-2 scales
        float_scale = decompress_mx_scale(layer.weight_scale.data)
